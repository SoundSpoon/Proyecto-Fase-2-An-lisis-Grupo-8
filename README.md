...
El entrenamiento que se utilizó para poder hacer un algoritmo predictor de asma se basó en los síntomas principales que se pueden observar en un paciente con asma el cual utilizó los sintomas de cansancio, malestar de garganta, dificultad para respirar, tos seca y congestión nasal como los sintomás que debe de detectar el algoritmo para poder predecir si el paciente puede padecer de asma o no. La idea principal es que si el sistema detecta dichos sintomas, pueda llegar a detectar un patrón matemático que pueda predecir si un paciente presenta asma o no y así reducir los riesgos que podrían afectar a la salud del paciente.
Sin embargo, al realizar las pruebas necesarias para poder generar el algoritmo de predicción de asma se encontró que la estructura inicial de nuestro proyecto planteaba una clasificación de severidad dividida en tres etapas (Leve, Moderada y Grave), lo cual generaba una alta ambigüedad matemática en las fronteras de decisión y penalizaba severamente el rendimiento y el Área Bajo la Curva (AUC) de los modelos predictivos. Para solventar esta limitación y optimizar la sensibilidad clínica del algoritmo, se reestructuró metodológicamente el problema hacia una clasificación binaria, consolidando los estados clínicos en un target simplificado: paciente "Grave" (casos moderados y severos que requieren intervención prioritaria) frente a paciente "Estable". 

<img width="625" height="490" alt="image" src="https://github.com/user-attachments/assets/9110362e-ad3d-4e2c-ad4e-5aa30d74cf34" />
